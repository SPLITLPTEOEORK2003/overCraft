...
// #                                                                      #
// #                                                                      #
// #                    لا تنسي لايك والاشتراك في القناة                     #
// #                      [ By : Kahrbaa   ]                              #
// #                                                                      #
// #                                                                      #
// ########################################################################
...
client.on('ready', () => {
	// - 
  client.user.setActivity("Kahrbaa 1K",{type: 'WATCHING'});
  console.log('╔[════════════════════════════════════]╗');
  console.log('')
  console.log('            ╔[════════════]╗')
...
client.on('voiceStateUpdate', (old, now) => {
  const channel = client.channels.get('510212756360658961');
  const currentSize = channel.guild.members.filter(m => m.voiceChannel).size;
  const size = channel.name.match(/\[\s(\d+)\s\]/);
  if (!size) return channel.setName(`➥ overCraft Online [ ${currentSize} ].`);
  if (currentSize !== size) channel.setName(`➥ overCraft Online [ ${currentSize} ].`);
});
...
if (command == "say") {
let rank = message.guild.member(message.author).roles.find('name', '.ownership');
...
var jimp = require('jimp');// npm i jimp 
      ...
      client.on('guildMemberAdd', member => {
      const welcomer =  member.guild.channels.find('name', 'text');//اسم الروم الي يرحب فيه
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("510212756360658955");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("510212756360658959");
    if (!channel) {
